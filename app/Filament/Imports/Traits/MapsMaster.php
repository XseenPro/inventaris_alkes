...
trait MapsMaster
{
  ...
  protected function bootMasterMaps(): void
  {
    ...
    $this->jenisKodeMap = Jenis::pluck('id', 'kode_jenis')->filter()->all();
  }
  ...
  protected function normalizeRowKeys(array $row): array
  {
    ...
    foreach ($row as $k => $v) {
      ...
      $aliases = [
        ...
        // mapping dari header Excel Alkes
        'no'                    => 'no',
      ...
      // untuk key yang mengandung slash seperti "No AKL/AKD"
      $key = str_replace('/', '_', $key);
  ...
  {
    $jenis = Jenis::where('kode_jenis', $kode)->first();

    if (!$jenis) {
      $byName = Jenis::whereRaw('LOWER(nama_jenis)=?', ['hardware'])->first();
      if ($byName) {
  ...
  protected function resolveOrCreateJenisByName(string $nama): ?Jenis
  {
    ...
    $jenis = Jenis::whereRaw('LOWER(nama_jenis)=?', [$key])->first();
  ...
  protected function resolveJenisByExcelName(?string $nama): ?int
  {
    ...
    $jenis = Jenis::whereRaw('LOWER(nama_jenis)=?', [$n])->first();
